"""
askLLM service — Kubernetes-adapted version

Runs as a standalone K8s Deployment (not a JupyterHub-managed subprocess).
Still uses HubAuth to validate tokens from the JupyterLab extension.

Key differences from the Docker Compose version:
- Secrets read from /app/secrets/ (K8s volume mount) with fallbacks
- MongoDB host configurable via MONGO_HOST env var
- JUPYTERHUB_URL read from JUPYTERHUB_API_URL env var
"""
...
def read_secret(name, env_fallback=None):
    """Read secret from mounted volume, fall back to env var.

    In K8s, secrets are mounted at /app/secrets/ (to avoid conflicting
    with the service account token at /var/run/secrets/).
    Falls back to /run/secrets/ (Docker Swarm) and then env vars (local dev).
    """
    for base_path in ("/app/secrets", "/run/secrets"):
    ...
    if env_fallback:
        ...
        if val:
            return val
    raise RuntimeError(f"Secret {name} not found at /app/secrets/{name}, /run/secrets/{name}, or env {env_fallback}")
...
JUPYTERHUB_URL = os.environ.get('JUPYTERHUB_API_URL',
                                os.environ.get('JUPYTERHUB_URL', 'http://jupyterhub:8081/jupyterhub/hub'))
